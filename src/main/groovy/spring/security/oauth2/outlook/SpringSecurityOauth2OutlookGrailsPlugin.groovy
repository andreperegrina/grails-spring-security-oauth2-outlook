...
class SpringSecurityOauth2OutlookGrailsPlugin extends Plugin {
    ...
    def title = "Spring Security Oauth2 Outlook Provider" // Headline display name of the plugin
    def author = "Andre Peregrina"
    def authorEmail = ""
    def description = '''\
...
'''
    ...
    def documentation = "http://grails.org/plugin/spring-security-oauth2-outlook"
...
//    def scm = [ url: "http://svn.codehaus.org/grails-plugins/" ]
    ...
    Closure doWithSpring() {
        { ->
            ...
            if (!hasProperty('log')) {
                log = LoggerFactory.getLogger(SpringSecurityOauth2OutlookGrailsPlugin)
            }

            if (printStatusMessages) {
                println("Configuring Spring Security OAuth2 Outlook plugin...")
            }
            SpringSecurityUtils.loadSecondaryConfig('DefaultOAuth2OutlookConfig')
            if (printStatusMessages) {
                println("... finished configuring Spring Security OAuth2 Outlook\n")
            }
        }
    }
    ...
    void doWithApplicationContext() {
        ...
        def OutlookOAuth2Service outlookOAuth2Service = grailsApplication.mainContext.getBean('outlookOAuth2Service') as OutlookOAuth2Service
        try {
            oAuth2BaseService.registerProvider(outlookOAuth2Service)
        } catch (OAuth2Exception exception) {
            log.error("There was an oAuth2Exception", exception)
            log.error("OAuth2 Outlook not loaded")
        }
    }
